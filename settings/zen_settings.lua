...
function M.build(plugin)
    ...
    local function get_plugin_version()
        ...
        if src:sub(1, 1) == "@" then
            ...
            local plugin_root = this_file:match("^(.*)/settings/zen_settings%.lua$")
            if plugin_root then
    ...
    local function get_scroll_bar_style()
        return (type(config.zen_scroll_bar) == "table"
            and config.zen_scroll_bar.style) or "bar"
    end
    ...
    for _, entry in ipairs(scroll_bar_styles) do
        table.insert(scroll_bar_sub_items, {
            text = entry.text,
            checked_func = function()
                return get_scroll_bar_style() == entry.style
            end,
            radio = true,
            callback = function()
                ...
                sub_item_table = {
                    ...
                    },
                    {
                        ...
                        callback = function(touchmenu_instance)
                            ...
                            local display_face = (not current_face or current_face == "default") and footer_font or current_face
    ...
    table.insert(reader_items, {
        ...
        sub_item_table_func = function()
            ...
            local items = {}
            table.insert(items, font_submenu)
    ...
    -- -------------------------------------------------------------------------
    -- Sleep screen (passthrough to KOReader's native screensaver_menu + presets)
    -- -------------------------------------------------------------------------

    -- Resolve plugin icons directory for built-in presets.
    local _icons_dir
    do
        local src = debug.getinfo(1, "S").source or ""
        if src:sub(1, 1) == "@" then
    ...
    --- Capture the current KOReader sleep screen settings into a table.
    ...
    --- Apply a preset's settings to KOReader's G_reader_settings.
    local function apply_sleep_screen_preset(preset)
        ...
        if preset.screensaver_stretch_limit_percentage ~= nil then
            G_reader_settings:saveSetting("screensaver_stretch_limit_percentage", preset.screensaver_stretch_limit_percentage)
        end
    end
    ...
    local function build_preset_items()
        ...
        -- Item to save current settings as a new preset
        table.insert(items, {
        ...
        -- List all presets (built-in first, then user)
        for i, preset in ipairs(all) do
            ...
            table.insert(items, {
                ...
                separator = is_last or (is_builtin and all[i + 1] ~= nil and not all[i + 1].builtin),
            })
        end
    ...
    table.insert(global_items, {
        ...
        sub_item_table_func = function()
            local ok, items = pcall(dofile, "frontend/ui/elements/screensaver_menu.lua")
            local sub = (ok and type(items) == "table") and items or {}
            -- Append presets submenu
            table.insert(sub, {
                text = _("Presets"),
    ...
    -- -------------------------------------------------------------------------
    -- Plugin management (passthrough to KOReader's PluginLoader)
    -- -------------------------------------------------------------------------
    ...
    do
        ...
        if ok and type(patch_item) == "table" then
            table.insert(advanced_items, patch_item)
        end
    end
